...
Description of available options:
  ...
  --disk-inode-used   Reports allocated inode space
  --disk-inode-avail  Reports available inode space
...
if ($report_disk_inode && !$report_inode_util && !$report_inode_used && !$report_inode_avail) {
  exit_with_error("Disk path is provided but metrics to report disk inode are not specified.");
}
if (!$report_disk_inode && ($report_inode_util || $report_inode_used || $report_inode_avail)) {
  exit_with_error("Metrics to report disk inode are provided but disk path is not specified.");
}

# check that there is a need to monitor at least something
if (!$report_mem_util && !$report_mem_used && !$report_mem_avail
  && !$report_swap_util && !$report_swap_used && !$report_disk_inode)
{
...
# collect disk inode metrics
if ($report_disk_inode)
{
  my @dfi = `/bin/df -i $dfi_path`;
  shift @dfi;
  ...
  foreach my $line (@dfi)
  {
    ...
    if ($report_inode_util) {
      ...
      add_metric('InodeSpaceUtilization', 'Percent', $inode_util, $fsystem, $mount);
    }
    if ($report_inode_used) {
      add_metric('InodeSpaceUsed', 'Count', $inode_used, $fsystem, $mount);
    }
    if ($report_inode_avail) {
      add_metric('InodeSpaceAvailable', 'Count', $inode_avail, $fsystem, $mount );
    }
  }
}
